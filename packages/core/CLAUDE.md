...
This file provides guidance to Claude Code when working with the `@kos/core` package.
...
The core package provides shared TypeScript types and utilities used by the Electron app and shared packages. It serves as the type definition layer for the Kos monorepo.
...
| `AgentEvent` | Events from KosAgent during chat (incl. task_backgrounded, shell_backgrounded, task_progress) |
...
// Import utilities
import { generateMessageId, debug } from '@kos/core';

// Or import from specific subpaths
import type { Session } from '@kos/core/types';
```
...
2. **Phase 2:** Move storage logic to `@kos/core`
