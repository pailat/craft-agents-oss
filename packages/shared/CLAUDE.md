...
`@kos/shared` is the core business logic package for Kos. It contains:
- Agent implementation (KosAgent, session-scoped tools, permission modes)
...
```typescript
import { KosAgent, getPermissionMode, setPermissionMode } from '@kos/shared/agent';
import { loadStoredConfig, type Workspace } from '@kos/shared/config';
import { getCredentialManager } from '@kos/shared/credentials';
import { CraftMcpClient } from '@kos/shared/mcp';
import { loadWorkspaceSources, type LoadedSource } from '@kos/shared/sources';
import { loadStatusConfig, createStatus } from '@kos/shared/statuses';
import { resolveTheme } from '@kos/shared/config/theme';
...
```
src/
├── agent/              # KosAgent, session-scoped-tools, mode-manager, mode-types, permissions-config
...
### KosAgent (`src/agent/claude-agent.ts`)
...
- Workspace: `~/.kos/workspaces/{id}/permissions.json`
- Source: `~/.kos/workspaces/{id}/sources/{slug}/permissions.json`
...
**Storage:** `~/.kos/workspaces/{id}/statuses/config.json`
...
**Storage:**
- App: `~/.kos/theme.json`
- Workspace: `~/.kos/workspaces/{id}/theme.json`
...
All sensitive credentials (API keys, OAuth tokens) are stored in an AES-256-GCM encrypted file at `~/.kos/credentials.enc`. The `CredentialManager` provides the API for reading and writing credentials.
...
│     └── ~/.kos/workspaces/{ws}/sources/{slug}/          │
└─────────────────────────────────────────────────────────────────┘
                              │
...
Multi-workspace configuration stored in `~/.kos/config.json`. Supports:
...
### Sources (`src/sources/`)
Sources are external data connections (MCP servers, APIs, local filesystems). Stored at `~/.kos/workspaces/{id}/sources/{slug}/` with config.json and guide.md. Types: `mcp`, `api`, `local`, `gmail`.

## Dependencies

- `@kos/core` - Shared types
