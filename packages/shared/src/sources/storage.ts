 ...
 * @param workspaceRootPath - Absolute path to workspace folder (e.g., ~/.kos/workspaces/xxx)
 ...
 * (like kos-docs) that don't have filesystem folders.
 */
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      // Currently only kos-docs is a builtin source
      if (slug === 'kos-docs') {
 ...
 * Built-in sources (like kos-docs) are always available and merged
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search kos-docs MCP for service-specific guidance)
