...
import type { StoredAttachment, ContentBadge } from '@kos/core'
import { normalizePath } from '@kos/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .kos workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.kos\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
