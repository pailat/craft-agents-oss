...
$DOWNLOAD_DIR = "$env:TEMP\kos-install"
$APP_NAME = "Kos"
...
$process = Get-Process -Name "Kos" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing Kos..."
...
Write-Info "Adding 'kos' command to PATH..."

$binDir = "$env:LOCALAPPDATA\Kos\bin"
$cmdFile = "$binDir\kos.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\Kos\Kos.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'kos' command)"
} else {
    Write-Success "Command 'kos' is ready"
}
...
Write-Host ""
Write-Host "  Kos has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: kos (restart terminal first)"
Write-Host ""
}
