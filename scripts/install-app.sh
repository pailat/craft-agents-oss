...
DOWNLOAD_DIR="$HOME/.kos/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.lukilabs.kos"
    if pgrep -x "Kos" >/dev/null 2>&1; then
        info "Quitting Kos..."
    ...
    echo ""
    printf "%b\n" "  Kos has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.kos/app"
    WRAPPER_PATH="$INSTALL_DIR/kos"
    APPIMAGE_INSTALL_PATH="$APP_DIR/Kos-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "Kos.*AppImage" >/dev/null 2>&1; then
        info "Stopping Kos..."
        pkill -f "Kos.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# Kos launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.kos/app/Kos-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@kos"
ELECTRON_CACHE_ALT="$HOME/.cache/@kos"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: Kos not found at $APPIMAGE_PATH"
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/Kos-x64.AppImage"
