...
// Detects instance number from folder name suffix (e.g., kos-1 → instance 1)
function detectInstance(): void {
  // Don't override if already set (e.g., by sourcing detect-instance.sh first)
  if (process.env.KOS_VITE_PORT) return;
  ...
  if (match) {
    ...
    process.env.KOS_INSTANCE_NUMBER = instanceNum;
    process.env.KOS_VITE_PORT = `${instanceNum}173`;
    process.env.KOS_APP_NAME = `Kos [${instanceNum}]`;
    process.env.KOS_CONFIG_DIR = join(process.env.HOME || "", `.kos-${instanceNum}`);
    process.env.KOS_DEEPLINK_SCHEME = `kos${instanceNum}`;
    console.log(`🔢 Instance ${instanceNum} detected: port=${process.env.KOS_VITE_PORT}, config=${process.env.KOS_CONFIG_DIR}`);
  }
}
...
function getElectronEnv(): Record<string, string> {
  const vitePort = process.env.KOS_VITE_PORT || "5173";
  ...
  return {
    ...
    KOS_CONFIG_DIR: process.env.KOS_CONFIG_DIR || "",
    KOS_APP_NAME: process.env.KOS_APP_NAME || "Kos",
    KOS_DEEPLINK_SCHEME: process.env.KOS_DEEPLINK_SCHEME || "kos",
    KOS_INSTANCE_NUMBER: process.env.KOS_INSTANCE_NUMBER || "",
  };
}
