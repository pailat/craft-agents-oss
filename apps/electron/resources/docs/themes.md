...
This guide explains how to customize the visual theme of Kos.

## Overview

Kos uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.kos/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.kos/theme.json` - Override specific colors (app-level)
...
Create `~/.kos/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.kos/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.kos/themes/{name}.json`
...
1. Create `~/.kos/theme.json` for overrides or `~/.kos/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.kos/theme.json` for overrides, `~/.kos/themes/` for presets)
