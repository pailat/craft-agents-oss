...
This guide explains how to configure sources (MCP servers, APIs, local filesystems) in Kos.
...
**Before doing anything else**, search for a specialized guide using the kos-docs MCP:

```
mcp__kos-docs__SearchKos({ query: "{service} source setup" })
```
...
Agent: [FIRST: Searches for Linear guide]
       mcp__kos-docs__SearchKos({ query: "linear source setup" })
...
- `~/.kos/workspaces/{workspaceId}/sources/{sourceSlug}/`
...
- `requiredPermissions` (optional): macOS permissions the MCP server needs. When specified, Kos shows actionable error messages if the app hasn't been granted these permissions in System Settings > Privacy & Security.
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.kos/workspaces/{ws}/sources/my-source
   ```
