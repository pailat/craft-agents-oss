...
This guide explains how to create and configure skills in Kos.
...
Kos uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in Kos
...
1. **Workspace skill checked first** - If `~/.kos/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.kos/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.kos/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.kos/workspaces/{ws}/skills/commit/SKILL.md`
