...
The primary desktop interface for Kos, built with Electron + React. Provides a multi-session inbox with chat interface for interacting with Claude via Craft workspaces.
...
│   │   ├── sessions.ts    # Session management, KosAgent integration
...
The `AgentEvent` types from `KosAgent` use different property names than you might expect:
...
### 4. KosAgent Constructor

`KosAgent` expects the full `Workspace` object, not just the ID:

```typescript
// Wrong:
new KosAgent({ workspaceId: workspace.id, model })

// Correct:
new KosAgent({ workspace, model })
```
...
**How it works:**
- `.env.1password` contains `op://` references to the `Dev_Kos` vault
...
```
kos://settings
kos://allSessions/session/session123
kos://sources/source/github
kos://action/new-chat
kos://workspace/{id}/allSessions/session/abc123
```
...
| `main/sessions.ts` | KosAgent wrapper, event processing, source integration |
